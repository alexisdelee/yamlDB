...
GtkTextBuffer * queryResult = NULL;
GtkTextBuffer * queryRecuperator = NULL;

int main (int    argc, char **argv)
{
    GtkWidget *window = NULL, *principalContainer = NULL, *secondaryContainer = NULL, *textArea = NULL, *textAreaResult = NULL;
    ...
    queryRecuperator = gtk_text_view_get_buffer (GTK_TEXT_VIEW (textArea));
    queryResult = gtk_text_view_get_buffer(GTK_TEXT_VIEW(textAreaResult));
    buttonLaunch = gtk_button_new_with_label ("Executer");
    ...
    g_signal_connect (G_OBJECT (buttonLaunch), "clicked", G_CALLBACK (callback_resultQuery), NULL);
